...
export const RELEASES: ReleaseEntry[] = [
  {
    ...
    features: [
      ...
      "**`/datasets/view?id=<id>` detail page** — inline-editable name + description (same pencil pattern as F2/F3), summary cards, splits table, classes pills, recent runs list (top 10 with links to each run's detail), action row (Re-split / Use for training / Delete). Query-param URL + Suspense wrapper for static-export compat.",
      "**`Pick from library` tab on `/train/dataset`** — tab toggle above the dropzone. Click 'Use this' on any library row to skip the upload step and jump straight to inspect-and-confirm with the dataset pre-selected.",
      "**`Name this dataset (optional)` card on `/train/dataset`** in the Drop-a-folder tab — two inputs (Name + Description) above the dropzone; values ride on the multipart upload's query string.",
      "**Inline rename on every library row** — hover-to-reveal pencil icon next to the dataset name swaps it for an input with Save/Cancel + Enter/Escape shortcuts. Saves immediately via PATCH; lets users rename all 45 backfilled rows from one screen instead of opening each detail page.",
      "**3 new backend routes**: `GET /api/datasets` (paginated list cross-referenced with HistoryDb for `last_used_at` + `run_count`; separates partial/inspect-failed folders into a `partial: []` list), `DELETE /api/datasets/{id}` (wipes folder + meta row; 409 Conflict with run names if any active training job is using it; preserves F3 history rows + library checkpoints), `PATCH /api/datasets/{id}` (F2-style name + description editing).",
      "**`POST /api/datasets/inspect` extended** with optional `?name=&description=` query params — backend upserts the meta row right after `inspect_dataset()` succeeds.",
      "**Soft-mention delete modal** — when `run_count > 0`, the confirmation shows an amber warning row: 'N training runs reference this dataset. History records stay; saved checkpoints stay in /models. Delete them separately from there if needed.'",
      "**Separate 'Couldn't read' section** below the main library table for folders where `inspect_dataset()` raises. Partial rows are still deletable so users can reclaim orphan disk space.",
      "**`/train/history` dataset filter** upgraded to use friendly names from `/api/datasets` instead of raw UUID stubs. Orphan dataset_ids (referenced by history rows whose folder has been deleted) show with a '(deleted)' suffix. `?dataset=<id>` URL param prefills the filter; page now wrapped in Suspense for static-export `useSearchParams` compatibility.",
      "**`JobManager.list_active_jobs_for_dataset(id)` helper** + the 409 delete guard that uses it.",
      "**27 new backend tests** covering schema v2 migration, HistoryDb meta methods, all three new routes (200/204/404/409 paths), `dataset_stats()` aggregator. 116 total backend tests across the project, all green.",
    ],
    fixes: [],
    knownLimitations: [
      "No bulk-rename or bulk-delete on the library table. Each row's actions are one-at-a-time. The hover-to-reveal pencil makes per-row rename fast, but a clinician with 45 datasets named `Dataset <stub>` still clicks 45 pencils. Bulk-edit is a future polish item.",
      ...
      "The `_record_to_info()` duplication between `routers/models.py` and `routers/training.py` is still tolerated (carried forward from F2 + F3).",
    ],
  },
  {
    version: "0.13.0",
