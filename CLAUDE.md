...
- ⏳ **F4 next** — Dataset library: reuse + grouping by dataset (last F-chain item; per `docs/FUTURE-FEATURES.md` item 4). Medium (3–5 days); reads off F3's history rows for `last_used_at` + `run_count`. After F4, the F-chain is complete and **P7 — Polish** is next per PLAN.md §14.
