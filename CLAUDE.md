...
- ⏳ **F6b next** — XAI part 2: reports (PDF/XLSX "include explanations") + `/models` "Test explanation" CTA + `/settings` Explanations section + folder-batch explanations. Then **P7 — Polish** (per PLAN.md §14): UX pass + clinician-readable error states + in-app help + docs site (incl. two Roboflow walkthroughs). After P7: P8 (macOS packaging) → P9 (Windows packaging) → P10 (Pilot). Phase tag at P7 completion: `v0.16-p7-polish` (F6 shifted the numbering). Pilot test (`docs/PILOT-TEST.md`) remains the v1.0 gate and still hasn't been run with a clinician + dataset.
