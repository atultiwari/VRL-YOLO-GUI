...
- ✅ **F6a** — Explainable AI: Eigen-CAM "Why?" heatmaps on Predict · `v0.15-f6a-explain` (unreleased) — XAI requested ahead of P7, re-opening the Future-Features chain (FUTURE-FEATURES §6). Planned in `docs/PLAN-F6a.md`; decisions signed off 2026-06-06: **pure in-house Eigen-CAM** (no dependency — gradient-free, ~80 LOC of NumPy/OpenCV) + **F6a/F6b split**. New `engine/explain.py` (Eigen-CAM: forward-hook the ultralytics `YOLO` object so it self-preprocesses → first principal component of `model.model.model[-2]` activations → JET RGBA heatmap; deterministic sign-orientation fixes a latent ReLU-zeroing bug the reference repos leave to luck; last-`Conv2d` `degraded` fallback for odd imports) + `InferenceEngine.explain_single` + `POST /api/inference/explain` + `components/predict/why-modal.tsx` (overlay + client-side opacity slider; detect = per-box stepper with peak stat, classify = one image-level map). **Eigen-CAM is class-agnostic → classify has no per-class switcher** (would need Grad-CAM; deferred). Copy says "where the model looked", never "saw class X here". Credits jacobgil/pytorch-grad-cam + rigvedrs/YOLO-26-CAM (both MIT) in `NOTICE`. 13 new tests = **130 backend** all green; tsc + 16-page static export green.
- ✅ **F6b** — XAI part 2: opacity setting + Models "Test explanation" · `v0.16-f6b-explain-models` (unreleased) — planned in `docs/PLAN-F6b.md`; **frontend-only, builds on F6a's engine + `WhyModal`, no backend change**. New `components/models/test-explanation-modal.tsx` + a *Test* button on every `/models` card (drop a sample image → `inferSingle` → `WhyModal` overlay; read-only sanity-check that a trained model looks at the right features before clinical use). New `/settings` **Explanations** section with `explain_default_opacity` (default 0.55) that `WhyModal` adopts as its initial opacity (adopts the saved value once `useSettings` hydrates, guarded so it doesn't clobber a live drag). **Scope call (made autonomously):** the report (PDF/XLSX) overlay embedding + folder-batch explanations were **deferred to F6c** — the PDF two-up reflow in `engine/reports.py` is the one XAI piece that wants human visual review, not a blind autonomous build. `tsc` + 16-page static export green; backend unchanged at 130.
- ⏳ **F6c next** — XAI part 3: report (PDF/XLSX) "Include explanations" (side-by-side source/overlay in PDF + overlay-thumbnail column in XLSX; needs the user's eyes on the rendered page) + folder-batch explanations (`?explain=true` + "+~Ns to the batch" estimate) + the "include explanations in reports by default" setting. Then **P7 — Polish** (per PLAN.md §14): UX pass + clinician-readable error states + in-app help + docs site (incl. two Roboflow walkthroughs). After P7: P8 (macOS packaging) → P9 (Windows packaging) → P10 (Pilot). Phase tags shifted by the F6 chain (F6a=`v0.15`, F6b=`v0.16`, F6c=`v0.17`, P7≈`v0.18-p7-polish`). Pilot test (`docs/PILOT-TEST.md`) remains the v1.0 gate and still hasn't been run with a clinician + dataset.
